...
int main(int argc, char** argv){
  ...
  TH2F* h3 = new TH2F("h3","h3",100,-20.0,20.0,100,-5.0,30.0);
  h3->GetXaxis()->SetTitle("x (cm)");
  h3->GetYaxis()->SetTitle("y (cm)");
  
  TBox* b1 = new TBox(-2.5,19.5,2.5,20.5);
  b1->SetFillStyle(0);
  b1->SetLineWidth(2);
  b1->SetLineColor(1);
  
  TBox* b2 = new TBox(-15.0,-0.5,15.0,0.5);
  b2->SetFillStyle(0);
  b2->SetLineWidth(2);
  b2->SetLineColor(1);
  ...
  for(int i=0 ; i<ent ; i++ ){
	...
	TF1* trackf = new TF1("track","[0]+[1]*x",-20.0,20.0);
	double a,b;
	b = L*100.0;
	a = -L*100.0/pos;
	trackf->SetParameters(b,a);
	//cout << a << " " << b << endl;
	
	TMarker* mk = new TMarker(pos,0.0,29);
	mk->SetMarkerSize(3);
	mk->SetMarkerColor(6);
